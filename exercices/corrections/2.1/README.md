...
1. Effectuez les configurations minimales à l'aide la commande `git config`
...
2. Utilisez la commande `git config --help` pour voir la documentation complète de cette commande
```bash
git config --help
```
- `--help` sur n'importe quelle commande `git` donne la documentation de celle-ci
3. Effectuez la commande `git config --list` pour voir le résultat
```bash
git config --list
```
...
PS : normalement les dossiers admin/ config/ et img/ ne devraient pas être présent dans le dépôt distant, ils sont présents à cause de l'option -f du `git add -f` qui permet malgré la présence du fichier ou du dossier dans le *.gitignore* de le versionner